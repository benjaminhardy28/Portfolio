...
import com.platform.PlatformerGame.Entities.Anchor;
import com.platform.PlatformerGame.Entities.Entity;
import com.platform.PlatformerGame.Entities.MonitoringVillian;
import com.platform.PlatformerGame.Entities.Player;
import com.platform.PlatformerGame.Entities.Surroundings;
import com.platform.PlatformerGame.Levels.LevelManager;
import com.platform.PlatformerGame.Physics.Collisions;
import com.platform.PlatformerGame.main.Game;
import com.platform.PlatformerGame.ApiRequests.PlayerInfo;
import static com.platform.PlatformerGame.utilz.Constants.Directions.*;
import com.platform.PlatformerGame.ApiRequests.ApiConnection;
...
public class Playing extends State implements Statemethods{
    ...
    //Draws the entities based on their current positioning held in their classes, which are all of type entity
    ...
    public void updateEntityCollisions(Player x, MonitoringVillian y, Anchor a){
        ...
        if (myPlayer.getHitBox().intersects(myvillain.getHitBox()) || myPlayer.getHitBox().getMaxY() == myvillain.getHitBox().getY() && myPlayer.getHitBox().getMaxX() >= myvillain.getHitBox().getX() && myPlayer.getHitBox().getX() <= myvillain.getHitBox().getMaxX() || myvillain.getHitBox().getMaxY() == myPlayer.getHitBox().getY() && myvillain.getHitBox().getMaxX() >= myPlayer.getHitBox().getX() && myvillain.getHitBox().getX() <= myPlayer.getHitBox().getMaxX()) {
            player.playerDies();
            PlayerInfo userInfo = api.getUserInfo(game.menuFrame.nameField.getText());
            if (userInfo == null) //if the user does exist, create a new one
                userInfo = new PlayerInfo();
            userInfo.updateLosses(1);
            userInfo.updateName(game.menuFrame.nameField.getText());
            game.updatePlayerInfo(userInfo);
