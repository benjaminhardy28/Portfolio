...
class NotificationBuilder:
    ...
    def startMonitoringSchedule(self):
        ...
        scheduler.add_job(self.check_if_update, 'interval', seconds=2)
        scheduler.start()
    ...
    def check_if_update(self):
        ...
        if self.new_hash != self.previous_hash:
            ...
            with open('previous_web_text.txt', 'w') as f:
    ...
    def add_notif(self, notif):
        ...
        data = {
            ...
            "notif_description" : "Playstation 5 information.",
            "full_notification" : notif,
            "short_notification" : notif
        }
        response = requests.post("http://127.0.0.1:5000/notification_from_python/13/20", json=data)
        print(response.text)
